...
epatterns.append('([A-Za-z.]+)\sat\s(([A-Za-z.]+)\.([A-Za-z]+))\.EDU') #cheriton
epatterns.append('([A-Za-z]+)\sWHERE\s([A-Za-z]+)\sDOM\sedu') #engler
epatterns.append('([A-Za-z]+\.[A-Za-z]+)\s\([A-Za-z]+\s[A-Za-z]+\s"@([A-Za-z]+)\.edu') #ouster
epatterns.append('([A-Za-z]+)\sAT\s([A-Za-z]+)\sDOT\sedu') #subh
epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+\.[A-Za-z]+)\.edu') #lam

# fail dabo epatterns.append('([A-Za-z]+)\s@\s([A-Za-z]+\.[A-Za-z]+)\.edu')
# fail dlwh epatterns.append('([A-Za-z]+\-[A-Za-z]+\-[A-Za-z]+\-[A-Za-z]+)\-@(\-[A-Za-z]+\-[A-Za-z]+\-[A-Za-z]+\-[A-Za-z]+\-[A-Za-z]\-[A-Za-z]\-[A-Za-z]\-[A-Za-z])\-\.\-e\-d\-u')
# fail hager epatterns.append('([A-Za-z]+)\sat\s(([A-Za-z]+)\s([A-Za-z]+)\s([A-Za-z]+))\sdot\sedu')
# fail jks epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+\;[A-Za-z]+)\;edu')
# fail latombe epatterns.append('([A-Za-z]+)@([A-Za-z]+\.[A-Za-z]+)\.edu')
# fail manning epatterns.append('([A-Za-z]+)\s([A-Za-z]+\.[A-Za-z]+)\.edu')
# fail pal epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+)\s([A-Za-z]+)\sedu')
# fail pal epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+\s[A-Za-z]+)\sedu')
# fail vladlen epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+)\sdot\sedu')
...
ppatterns.append('\+\d\s(\d{3})\s(\d{3})\s(\d{4})')
ppatterns.append('\[(\d{3})]\s(\d{3})-(\d{4})') #nass
ppatterns.append('\+\d\s\((\d{3})\)\s(\d{3})-(\d{4})')#manning
ppatterns.append('\((\d{3})\)(\d{3})-(\d{4})') #ashishg
ppatterns.append('\((\d{3})\)\s(\d{3})-(\d{4})') # bgirod
ppatterns.append('\+\d\s(\d{3})\s(\d{3})-(\d{4})') #shoham
